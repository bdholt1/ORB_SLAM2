...
public:
   ...
   */
  inline double  score(const BowVector& a,
                       const BowVector& b) const;
   ...
   *   word id
   */
  virtual NodeId getParentNode(WordId wid,
                               int    levelsup) const;
  ...
  const std::vector<TDescriptor>& features, BowVector& v) const
{
  ...
  if ((m_weighting == TF) || (m_weighting == TF_IDF))
  {
    ...
    if (!v.empty() && !must)
    {
      ...
      for (BowVector::iterator vit = v.begin(); vit != v.end();
           vit++) vit->second /= nd;
    }
  }
  else // IDF || BINARY
  ...
  else
  {
    ...
    while (!parents.empty())
    {
      ...
      for (cit = child_ids.begin(); cit != child_ids.end(); ++cit)
      {
        ...
        if (child_node.isLeaf()) words.push_back(child_node.word_id);
        else parents.push_back(*cit);
      } // for each child
    }   // while !parents.empty
  }
}
  ...
  const std::string& filename)
{
  ...
  if ((m_k < 0) || (m_k > 20) || (m_L < 1) || (m_L > 10) || (n1 < 0) ||
      (n1 > 5) || (n2 < 0) || (n2 > 3))
  {
...
#endif // ifndef __D_T_TEMPLATED_VOCABULARY__
