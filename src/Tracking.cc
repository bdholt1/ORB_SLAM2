...
Tracking::Tracking(System           *pSys,
  ...
  mpKeyFrameDB(pKFDB), mpInitializer(static_cast<Initializer *>(NULL)), mpSystem(
    pSys), mpViewer(NULL),
  ...
  mnLastRelocFrameId(0)
{
  ...
  if (sensor == System::STEREO) mpORBextractorRight = new ORBextractor(nFeatures,
                                                                       fScaleFactor,
                                                                       nLevels,
                                                                       fIniThFAST,
                                                                       fMinThFAST);

  if (sensor == System::MONOCULAR) mpIniORBextractor = new ORBextractor(
      2 * nFeatures,
      fScaleFactor,
      nLevels,
      fIniThFAST,
                                ...
                                const double & timestamp)
{
  ...
  if ((fabs(mDepthMapFactor - 1.0f) > 1e-5) ||
      (imDepth.type() != CV_32F)) imDepth.convertTo(imDepth, CV_32F,
                                                    mDepthMapFactor);
...
cv::Mat Tracking::GrabImageMonocular(const cv::Mat& im, const double& timestamp)
{
  ...
  if ((mState == NOT_INITIALIZED) ||
      (mState == NO_IMAGES_YET)) mCurrentFrame = Frame(mImGray,
                                                       timestamp,
                                                       mpIniORBextractor,
                                                       mpORBVocabulary,
                                                       mK,
                                                       ...
                                                       mbf,
                                                       mThDepth);
  else mCurrentFrame = Frame(mImGray,
                             timestamp,
                             mpORBextractorLeft,
                             mpORBVocabulary,
                             mK,
...
void Tracking::Track()
{
  ...
  if (mState == NOT_INITIALIZED)
  {
    if ((mSensor == System::STEREO) ||
        (mSensor == System::RGBD)) StereoInitialization();
  ...
  else
  {
    ...
    if (bOK)
    {
      ...
      if (!mLastFrame.mTcw.empty())
      {
        ...
        mLastFrame.GetRotationInverse().copyTo(LastTwc.rowRange(0, 3).colRange(0,
                                                                               3));
      ...
      for (int i = 0; i < mCurrentFrame.N; i++)
      {
        if (mCurrentFrame.mvpMapPoints[i] &&
            mCurrentFrame.mvbOutlier[i]) mCurrentFrame.mvpMapPoints[i] =
...
void Tracking::MonocularInitialization()
{
  if (!mpInitializer)
  {
    ...
    if (mCurrentFrame.mvKeys.size() > 100)
    {
      ...
      for (size_t i = 0; i < mCurrentFrame.mvKeysUn.size();
           i++) mvbPrevMatched[i] = mCurrentFrame.mvKeysUn[i].pt;
...
bool Tracking::TrackWithMotionModel()
{
  ...
  if (mSensor != System::STEREO) th = 15;
  else th = 7;
...
bool Tracking::TrackLocalMap()
{
        ...
        if (!mbOnlyTracking)
        {
          if (mCurrentFrame.mvpMapPoints[i]->Observations() >
              0) mnMatchesInliers++;
               ...
               System::STEREO) mCurrentFrame.mvpMapPoints[i] =
          static_cast<MapPoint *>(NULL);
    }
  }
  ...
  if ((mCurrentFrame.mnId < mnLastRelocFrameId + mMaxFrames) &&
      (mnMatchesInliers < 50)) return false;
...
bool Tracking::NeedNewKeyFrame()
{
  ...
  if (mSensor != System::MONOCULAR)
  {
    for (int i = 0; i < mCurrentFrame.N; i++)
    {
      ...
      {
        if (mCurrentFrame.mvpMapPoints[i] &&
            !mCurrentFrame.mvbOutlier[i]) nTrackedClose++;
  ...
  const bool c1b =
    (mCurrentFrame.mnId >= mnLastKeyFrameId + mMinFrames && bLocalMappingIdle);
    ...
    if (mCurrentFrame.mvpMapPoints[i])
    {
      ...
      if (!pMP->isBad())
      {
        ...
        for (map<KeyFrame *, size_t>::const_iterator it = observations.begin(),
             ...
             it != itend; it++) keyframeCounter[it->first]++;
      }
      else
      {
    ...
    for (int i = 0; i < nKFs; i++)
    {
      ...
      if (!Tcw.empty())
      {
        ...
        for (int io = 0; io < mCurrentFrame.N; io++)
          if (mCurrentFrame.mvbOutlier[io]) mCurrentFrame.mvpMapPoints[io] =
              static_cast<MapPoint *>(NULL);
          ...
          if (nadditional + nGood >= 50)
          {
            ...
            if ((nGood > 30) && (nGood < 50))
            {
              ...
              for (int ip = 0; ip < mCurrentFrame.N; ip++)
                if (mCurrentFrame.mvpMapPoints[ip]) sFound.insert(
