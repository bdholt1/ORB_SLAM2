    ...
    for (size_t iG = 0, iendG = mvConsistentGroups.size(); iG < iendG; iG++)
    {
      ...
      if (bConsistent)
      {
        ...
        if (!vbConsistentGroup[iG])
        {
          ...
          vbConsistentGroup[iG] = true; // this avoid to include the same group
                                        // more than once
        }
...
bool LoopClosing::ComputeSim3()
{
  ...
  if (!bMatch)
  {
    for (int i = 0; i < nInitialCandidates;
         i++) mvpEnoughConsistentCandidates[i]->SetErase();
    mpCurrentKF->SetErase();
    return false;
  }
  ...
  if (nTotalMatches >= 40)
  {
    for (int i = 0; i < nInitialCandidates; i++)
      if (mvpEnoughConsistentCandidates[i] !=
          mpMatchedKF) mvpEnoughConsistentCandidates[i]->SetErase();
    return true;
  }
  else
  {
...

  {
         ...
         mit != mend; mit++)
    {
      ...
      for (size_t iMP = 0, endMPi = vpMPsi.size(); iMP < endMPi; iMP++)
      {
        ...
        Eigen::Matrix<double, 3, 1> eigCorrectedP3Dw = g2oCorrectedSwi.map(g2oSiw.map(
                                                                             eigP3Dw));
