                                     ...
                                     int    maxIterations)
{
  ...
  else nIterations = ceil(log(1 - mRansacProb) / log(1 - pow(epsilon, 3)));
...
void Sim3Solver::ComputeSim3(cv::Mat& P1, cv::Mat& P2)
{
  ...
  cv::eigen(N, eval, evec);                 // evec[0] is the quaternion of the
                                            // desired rotation

  cv::Mat vec(1, 3, evec.type());
  (evec.row(0).colRange(1, 4)).copyTo(vec); // extract imaginary part of the
                                            // quaternion (sin*axis)

  // Rotation angle. sin is the norm of the imaginary part, cos is the real part
  double ang = atan2(norm(vec), evec.at<float>(0, 0));

  vec = 2 * ang * vec / norm(vec); // Angle-axis representation. quaternion
                                   // angle is the half
