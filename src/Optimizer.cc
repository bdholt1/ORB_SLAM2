                                 ...
                                 const bool bRobust)
{
  ...
  for (size_t i = 0; i < vpMP.size(); i++)
  {
         ...
         mit != observations.end(); mit++)
    {
      ...
      if (pKF->mvuRight[mit->second] < 0)
      {
        ...
        e->setVertex(0,
                     dynamic_cast<g2o::OptimizableGraph::Vertex *>(optimizer.
                                                                   vertex(id)));
        e->setVertex(1,
                     dynamic_cast<g2o::OptimizableGraph::Vertex *>(optimizer.
                                                                   vertex(
  ...
  for (size_t i = 0; i < vpMP.size(); i++)
  {
    ...
    g2o::VertexSBAPointXYZ *vPoint =
      static_cast<g2o::VertexSBAPointXYZ *>(optimizer.vertex(pMP->mnId + maxKFid +
                                                             1));
                                       ...
                                       const bool& bFixScale)
{
  ...
  for (size_t i = 0, iend = vpKFs.size(); i < iend; i++)
  {
    ...
    else
    {
      ...
      Eigen::Matrix<double, 3, 1> tcw = Converter::toVector3d(
        pKF->GetTranslation());
  ...
  const Eigen::Matrix<double, 7,
                      7> matLambda = Eigen::Matrix<double, 7, 7>::Identity();

  // Set Loop edges
  for (map<KeyFrame *, set<KeyFrame *> >::const_iterator mit =
         LoopConnections.begin(), mend = LoopConnections.end();
       mit != mend; mit++)
  {
    KeyFrame *pKF                         = mit->first;
         ...
         sit != send; sit++)
    {
      ...
      if (((nIDi != pCurKF->mnId) || (nIDj != pLoopKF->mnId)) &&
          (pKF->GetWeight(*sit) < minFeat)) continue;
  ...
  for (size_t i = 0, iend = vpKFs.size(); i < iend; i++)
  {
    ...
    if (iti != NonCorrectedSim3.end()) Swi = (iti->second).inverse();
    ...
    if (pParentKF)
    {
      ...
      LoopClosing::KeyFrameAndPose::const_iterator itj = NonCorrectedSim3.find(
        pParentKF);

      if (itj != NonCorrectedSim3.end()) Sjw = itj->second;
      else Sjw = vScw[nIDj];
         ...
         sit != send; sit++)
    {
      ...
      if (pLKF->mnId < pKF->mnId)
      {
        ...
        LoopClosing::KeyFrameAndPose::const_iterator itl = NonCorrectedSim3.find(
          pLKF);

        if (itl != NonCorrectedSim3.end()) Slw = itl->second;
        else Slw = vScw[pLKF->mnId];
         ...
         vit != vpConnectedKFs.end(); vit++)
    {
          ...
          !sLoopEdges.count(pKFn))
      {
        if (!pKFn->isBad() && (pKFn->mnId < pKF->mnId))
        {
          ...
          if (itn != NonCorrectedSim3.end()) Snw = itn->second;
          else Snw = vScw[pKFn->mnId];
  ...
  for (size_t i = 0, iend = vpMPs.size(); i < iend; i++)
  {
    ...
    Eigen::Matrix<double, 3,
                  1> eigCorrectedP3Dw = correctedSwr.map(Srw.map(eigP3Dw));
