                             ...
                             vector<bool>       & vbTriangulated)
{
  ...
  thread threadH(&Initializer::FindHomography, this, ref(vbMatchesInliersH), ref(
                   SH), ref(H));
  thread threadF(&Initializer::FindFundamental, this, ref(vbMatchesInliersF), ref(
                   SF), ref(F));
  ...
  // (0.40-0.45)
  if (RH > 0.40) return ReconstructH(vbMatchesInliersH,
                                     H,
                                     mK,
                                     R21,
                                     t21,
                                   ...
                                   vector<bool>& vbMatchesInliers, float sigma)
{
  ...
  for (int i = 0; i < N; i++)
  {
    ...
    const float squareDist1 = (u1 - u2in1) * (u1 - u2in1) + (v1 - v2in1) *
                              (v1 - v2in1);
    ...
    const float squareDist2 = (u2 - u1in2) * (u2 - u1in2) + (v2 - v1in2) *
                              (v2 - v1in2);
    ...
    if (bIn) vbMatchesInliers[i] = true;
    else vbMatchesInliers[i] = false;
  }
                               ...
                               int                  minTriangulated)
{
  ...
  // case d'=d2
  float aux_stheta = sqrt((d1 * d1 - d2 * d2) * (d2 * d2 - d3 * d3)) /
                     ((d1 + d3) * d2);
  ...
  // case d'=-d2
  float aux_sphi = sqrt((d1 * d1 - d2 * d2) * (d2 * d2 - d3 * d3)) /
                   ((d1 - d3) * d2);
  ...
  // We reconstruct all hypotheses and check in terms of triangulated points and
  // parallax
