...
ORBmatcher::ORBmatcher(float nnratio, bool checkOri) : mfNNratio(nnratio),
  ...
  for (size_t iMP = 0; iMP < vpMapPoints.size(); iMP++)
  {
    ...
    if (bestDist <= TH_HIGH)
    {
      if ((bestLevel == bestLevel2) &&
          (bestDist > mfNNratio * bestDist2)) continue;
                                       ...
                                       const KeyFrame     *pKF2)
{
  ...
  const float a = kp1.pt.x *
                  F12.at<float>(0, 0) + kp1.pt.y * F12.at<float>(1,
                                                                 0) +
                  F12.at<float>(2, 0);
  const float b = kp1.pt.x *
                  F12.at<float>(0, 1) + kp1.pt.y * F12.at<float>(1,
                                                                 1) +
                  F12.at<float>(2, 1);
  const float c = kp1.pt.x *
                  F12.at<float>(0, 2) + kp1.pt.y * F12.at<float>(1,
                                                                 2) +
                  F12.at<float>(2, 2);
  ...
  for (int iMP = 0, iendMP = vpPoints.size(); iMP < iendMP; iMP++)
  {
         ...
         vit != vend; vit++)
    {
      ...
      if ((kpLevel < nPredictedLevel - 1) ||
          (kpLevel > nPredictedLevel)) continue;
        ...
        for (size_t i2 = 0, iend2 = f2it->second.size(); i2 < iend2; i2++)
        {
          ...
          if (!bStereo1 && !bStereo2)
          {
            ...
            if (distex * distex + distey * distey <
                100 * pKF2->mvScaleFactors[kp2.octave]) continue;
          }
  ...
  for (int i = 0; i < nMPs; i++)
  {
    ...
    if (bestDist <= TH_LOW)
    {
      ...
      if (pMPinKF)
      {
        if (!pMPinKF->isBad())
        {
          if (pMPinKF->Observations() >
              pMP->Observations()) pMP->Replace(pMPinKF);
          else pMPinKF->Replace(pMP);
        }
      }
      else
      {
                             ...
                             const float         th)
{
  ...
  for (int i1 = 0; i1 < N1; i1++)
  {
         ...
         vit != vend; vit++)
    {
      ...
      if ((kp.octave < nPredictedLevel - 1) ||
          (kp.octave > nPredictedLevel)) continue;
  ...
  for (int i = 0; i < LastFrame.N; i++)
  {
    ...
    if (pMP)
    {
      if (!LastFrame.mvbOutlier[i])
      {
        ...
        if (bForward) vIndices2 = CurrentFrame.GetFeaturesInArea(u,
                                                                 v,
                                                                 radius,
                                                                 nLastOctave);
        else if (bBackward) vIndices2 = CurrentFrame.GetFeaturesInArea(u,
                                                                       v,
                                                                       radius,
                                                                       0,
                                                                       nLastOctave);
        else vIndices2 = CurrentFrame.GetFeaturesInArea(u,
                                                        v,
                                                        radius,
                                                        nLastOctave - 1,
                                                        nLastOctave + 1);
  ...
  for (size_t i = 0, iend = vpMPs.size(); i < iend; i++)
  {
    ...
    if (pMP)
    {
      if (!pMP->isBad() && !sAlreadyFound.count(pMP))
      {
        ...
        const vector<size_t> vIndices2 = CurrentFrame.GetFeaturesInArea(u,
                                                                        v,
                                                                        radius,
                                                                        nPredictedLevel - 1,
                                                                        nPredictedLevel +
