...
KeyFrame::KeyFrame(Frame& F, Map *pMap, KeyFrameDatabase *pKFDB) :
  ...
  mfGridElementWidthInv(F.mfGridElementWidthInv), mfGridElementHeightInv(
    F.mfGridElementHeightInv),
  ...
  mbf(F.mbf), mb(F.mb), mThDepth(F.mThDepth), N(F.N), mvKeys(F.mvKeys), mvKeysUn(
    F.mvKeysUn),
...
{
  {
    ...
    if (!mConnectedKeyFrameWeights.count(pKF)) mConnectedKeyFrameWeights[pKF] =
        weight;
    else if (mConnectedKeyFrameWeights[pKF] !=
             weight) mConnectedKeyFrameWeights[pKF] = weight;
    else return;
  }
...
void KeyFrame::UpdateBestCovisibles()
{
  ...
  for (map<KeyFrame *, int>::iterator mit = mConnectedKeyFrameWeights.begin(),
       ...
       mit != mend; mit++) vPairs.push_back(make_pair(mit->second, mit->first));
...
set<KeyFrame *>KeyFrame::GetConnectedKeyFrames()
{
  ...
  for (map<KeyFrame *, int>::iterator mit = mConnectedKeyFrameWeights.begin();
       mit != mConnectedKeyFrameWeights.end(); mit++) s.insert(mit->first);
  return s;
}
...
vector<KeyFrame *>KeyFrame::GetBestCovisibilityKeyFrames(const int& N)
{
  ...
  if ((int)mvpOrderedConnectedKeyFrames.size() <
      N) return mvpOrderedConnectedKeyFrames;
  else return vector<KeyFrame *>(mvpOrderedConnectedKeyFrames.begin(),
...
vector<KeyFrame *>KeyFrame::GetCovisiblesByWeight(const int& w)
{
  ...
  if (it == mvOrderedWeights.end()) return vector<KeyFrame *>();
  else
  {
...
int KeyFrame::GetWeight(KeyFrame *pKF)
{
  ...
  if (mConnectedKeyFrameWeights.count(pKF)) return mConnectedKeyFrameWeights[pKF];
  else return 0;
}
...
void KeyFrame::SetBadFlag()
{
  ...
  for (map<KeyFrame *, int>::iterator mit = mConnectedKeyFrameWeights.begin(),
       mend = mConnectedKeyFrameWeights.end();
       mit != mend; mit++) mit->first->EraseConnection(this);
