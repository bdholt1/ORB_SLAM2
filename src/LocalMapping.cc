...
void LocalMapping::Run()
{
  ...
  while (1)
  {
    ...
    if (CheckNewKeyFrames())
    {
      ...
      {
        // Local BA
        if (mpMap->KeyFramesInMap() > 2) Optimizer::LocalBundleAdjustment(
            mpCurrentKeyFrame,
            &mbAbortBA,
            mpMap);
  ...
  while (lit != mlpRecentAddedMapPoints.end())
  {
             ...
             (pMP->Observations() <= cnThObs))
    {
      ...
      lit = mlpRecentAddedMapPoints.erase(lit);
             ...
             3) lit = mlpRecentAddedMapPoints.erase(lit);
    else lit++;
  }
}
...
void LocalMapping::CreateNewMapPoints()
{
  ...
  for (size_t i = 0; i < vpNeighKFs.size(); i++)
  {
    ...
    for (int ikp = 0; ikp < nmatches; ikp++)
    {
      ...
      cv::Mat xn1 =
        (cv::Mat_<float>(3, 1) << (kp1.pt.x - cx1) * invfx1,
         (kp1.pt.y - cy1) * invfy1, 1.0);
      cv::Mat xn2 =
        (cv::Mat_<float>(3, 1) << (kp2.pt.x - cx2) * invfx2,
      ...
      if (bStereo1) cosParallaxStereo1 =
          cos(2 *
      ...
      else continue;  // No stereo and very low parallax
      ...
      if ((ratioDist * ratioFactor < ratioOctave) ||
          (ratioDist > ratioOctave * ratioFactor)) continue;
...
void LocalMapping::SearchInNeighbors()
{
       ...
       vit != vend; vit++)
  {
    ...
    if (pKFi->isBad() ||
        (pKFi->mnFuseTargetForKF == mpCurrentKeyFrame->mnId)) continue;
         ...
         vit2 != vend2; vit2++)
    {
      ...
      if (pKFi2->isBad() ||
          ...
          (pKFi2->mnId == mpCurrentKeyFrame->mnId)) continue;
       ...
       vitKF != vendKF; vitKF++)
  {
         ...
         vitMP != vendMP; vitMP++)
    {
      ...
      if (pMP->isBad() ||
          (pMP->mnFuseCandidateForKF == mpCurrentKeyFrame->mnId)) continue;
      ...
      if (pMP)
      {
        if (!pMP->isBad())
        {
          ...
          if (pMP->Observations() > thObs)
          {
            ...
            for (map<KeyFrame *, size_t>::const_iterator mit =
