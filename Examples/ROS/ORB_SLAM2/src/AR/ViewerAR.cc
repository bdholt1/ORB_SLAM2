...
cv::Mat ExpSO3(const float& x, const float& y, const float& z)
{
  ...
  if (d < eps) return I + W + 0.5f * W * W;
  else return I + W * sin(d) / d + W * W * (1.0f - cos(d)) / d2;
}
...
void ViewerAR::Run()
{
  ...
  pangolin::GlTexture imageTexture(w,
                                   h,
                                   GL_RGB,
                                   false,
                                   0,
  ...
  pangolin::OpenGlMatrixSpec P = pangolin::ProjectionMatrixRDF_TopLeft(w,
                                                                       h,
                                                                       fx,
                                                                       fy,
                                                                       cx,
...
void ViewerAR::AddTextToImage(const string& s,
                              cv::Mat     & im,
                              const int     r,
                              const int     g,
                              const int     b)
                              ...
                              const int                      iterations)
{
  ...
  for (int n = 0; n < iterations; n++)
  {
    ...
    for (int i = 0; i < N; i++)
    {
      vDistances[i] = fabs(vPoints[i].at<float>(0) * a + vPoints[i].at<float>(
                             1) * b + vPoints[i].at<float>(2) * c + d) * f;
    }
...
Plane::Plane(const std::vector<MapPoint *>& vMPs,
             const cv::Mat                & Tcw) : mvMPs(vMPs), mTcw(Tcw.clone())
{
